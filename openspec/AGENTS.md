...
### Optimization Patterns (December 2025)

When implementing performance optimizations, follow these established patterns:
...
stats = queryset.aggregate(
    count_a=Count('pk', filter=Q(status='active')),
    count_b=Count('pk', filter=Q(status='pending')),
)

# Use Exists() subqueries instead of values_list + exclude
from django.db.models import Exists, OuterRef

queryset = queryset.annotate(
    has_related=Exists(RelatedModel.objects.filter(parent_id=OuterRef('pk')))
).filter(has_related=False)

# Always add select_related for foreign keys in list views
...
# Create/Edit operations: 10/min user + 20/min IP
...
# Delete operations: 5/min user + 10/min IP
@login_required(login_url="user-login")
...
# In form clean methods
def clean_description(self):
    value = self.cleaned_data.get('description', '')
    return sanitize_text_input(value)

# In upload_to functions
def upload_path(instance, filename):
    clean_name = sanitize_filename(filename)
    return f"uploads/{instance.pk}/{clean_name}"
```
...
def clean(self):
    ...
    pog_weeks = cleaned_data.get('pog_wks')
    pog_days = cleaned_data.get('pog_days', 0)

    if birth_weight and pog_weeks:
        is_valid, message = validate_birth_weight_for_gestational_age(
            birth_weight, pog_weeks, pog_days, strict=False
        )
        if not is_valid:
            self.add_error('birth_weight', message)

    return cleaned_data
```

**Date Cross-Validation Pattern:**
```python
def clean(self):
    cleaned_data = super().clean()
    date_start = cleaned_data.get('date_of_onset')
    date_identified = cleaned_data.get('date_identified')
    date_resolved = cleaned_data.get('date_resolved')

    if date_start and date_identified and date_identified < date_start:
        raise ValidationError({
            'date_identified': 'Cannot be before date of onset.'
        })

    if date_start and date_resolved and date_resolved < date_start:
        raise ValidationError({
            'date_resolved': 'Cannot be before date of onset.'
        })
...
- [ ] All new models inherit from `TimeStampedModel, UserTrackingMixin`
- [ ] Choices added to `ndas/custom_codes/choice.py`
- [ ] Searchable fields have `db_index=True`
- [ ] Views use `get_object_or_404()` not `.objects.get()`
- [ ] CRUD operations have rate limiting decorators
- [ ] Text inputs use `sanitize_text_input()` in form clean methods
- [ ] File uploads use `sanitize_filename()` in upload_to functions
- [ ] Medical fields use appropriate validators from validators.py
- [ ] List views include `select_related()` for foreign keys
- [ ] Date fields with relationships have cross-validation

### Related Documentation

- `CLAUDE.md` - Main project documentation (includes Recent Optimizations section)
- `AGENTS.md` - AI agent workflow instructions
- `.github/copilot-instructions.md` - IDE assistant guidance
- `temp_documents/BUG_AND_PERFORMANCE_ANALYSIS.md` - Remaining known issues
- `temp_documents/BUG_FIX_PLAN.md` - Prioritized bug fix implementation plan
