...
- **SUPERSEDED (2026-09-12):** The human explicitly renegotiated this spec's frozen `birth_weight` boundaries in `spec-birth-weight-validation-range.md`: the POG-specific gestational-age-aware check wired into `Patient.clean()` by fix #3 above (`validate_birth_weight_for_gestational_age`, `BIRTH_WEIGHT_RANGES_BY_POG`, and their tests in `patients/tests/test_validators.py`) has been removed, and the basic field-level `validate_birth_weight` floor changed from 300g to 200g. This was a deliberate, informed decision after the removal was flagged against this frozen spec — not an accidental regression. Fixes #1, #2, and #4 above (DX_NORMAL, HINE thresholds, `getRC`) are unaffected and remain in force.
